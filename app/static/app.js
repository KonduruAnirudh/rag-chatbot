...
const state = {
  uploading: false,
  documentCount: 0,   // Pass 3 uses this to enable or disable chat
};
...
// One place that talks to the backend. Turns every failure into an
...
async function api(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    // FastAPI's own validation errors (422) return detail as a list
...
// textContent never interprets HTML, so filenames can't inject markup.
...
// estimates that keep a 30-second wait from looking frozen.
...
async function uploadFiles(fileList) {
  ...
  // One at a time — the backend takes one file per request
...
/* ================================================================
   Wiring: buttons, drag and drop, mobile drawer
   ================================================================ */
...
// dragenter/dragleave also fire when crossing child elements, so a
// counter tracks whether the pointer is still inside the sidebar.
let dragDepth = 0;
...
// navigate away and open it — losing the whole conversation.
